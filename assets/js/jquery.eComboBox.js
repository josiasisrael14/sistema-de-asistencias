...
(function( $ ){
  ...
  var methods = {
     init : function( options ) {
		...
		return this.each(function() { 
			...
			if (settings.allowNewElements){
				selectElement.prepend('<option>{Nuevo Elemento}</option>');
			}
			...
			selectElement.keydown( function(e){
				...
				if(e.keyCode == "46"){ // del-button
					if(selected != "{Nuevo Elemento}"){
						$(this).children("option:selected").remove();
					}
					return;
				}
				
				if( selected=="{Nuevo Elemento}" || settings['editableElements'] ) {
	 ...
	 enableAddingNewElements: function(){
		var fChild = $(this).children().first();
		if(fChild.text() != "{Nuevo Elemento}"){
			$(this).prepend('<option>{Nuevo Elemento}</option>');
		}
	 },
