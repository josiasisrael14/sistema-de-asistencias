...
<button class="btn btn-info" type="button" id="generareven" onclick="generar(6);">Generar</button>
        <input type="text" id="codigo_persona" name="codigo_persona" class="form-control input-sm" value="<?php echo $usuarios->codigo_persona ?>">
           ...
           <div class="form-group">
              <label for="tipodepartamento">Horarios</label>

      <select class="form-control" id="idhorario" name="idhorario" value="<?php echo $horario->idhorario ?>">


    <option value="">seleccione</option>
    <?php foreach ($horario as  $value):?>
<option value="<?php echo $value->idhorario;?>" <?php if ($value->idhorario==$usuarios->idhorario):?> selected  <?php endif?> > <?php echo $value->horaentrada;?>-<?php echo $value->horasalida;?></option> 
            <?php endforeach?>      
              </select>
  ...
  <script type="text/javascript">
   ...
   $("#btn_guardar_pago").on("click",function(){
    ...
    if(nombre.length==0||apellidos.length==0||login.length==0||email.length==0||password.length==0||codigo_persona.length==0||idhorario.length==0||idtipousuario.length==0||iddepartamento.length==0){
